...
|   |
│   ├── pmr-allocators.md         # C++17 PMR内存资源与分配器：std::pmr::memory_resource、
│   │                             # std::pmr::polymorphic_allocator、std::pmr::vector/string、内存池与自定义内存资源、
│   │                             # PMR的优势(减少堆分配次数/避免内存碎片/提高缓存命中率)
│   │
...
│   |
│   ├── span-and-mdspan.md        # std::span(C++20)：零开销视图(指针+长度)、
│   │
...
│   ├── diagnostic-and-debugging.md # 现代 C++ 诊断与运行时排障
│   │                             # std::stacktrace 捕获、static_assert 静态断言、C++23 [[assume]] 属性、编译期 ODR 违规检测
│   │
