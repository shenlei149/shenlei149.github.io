...
从 $s$ 点开始，遍历所有的边，遇到第一个没有访问过的顶点 $a$，访问点 $a$，然后遍历 $a$ 的所有边，遇到第一个没有访问过的顶点 $b$，访问点 $b$，然后遍历 $b$ 的所有边，以此类推。直到遇到一个顶点，其所有邻接边的另一个顶点都访问过了，然后返回上一层。
...
如果使用普通的堆来实现，有两个问题。第一个问题是需要将顶点和距离封装成一个结构体或者 `std::pair`，自定义比较运算符。第二个问题是不能删除指定的节点，也不能更新指定的节点，那么算法中的更新操作就是再添加一个节点进去，重复入堆。如果弹出来的顶点已经处理过了，即已经在 $X$ 里面，无需再次处理，这个过程也称为懒删除。重复入堆的本质是遍历边，那么堆的操作时间复杂度从 $\log n$ 变成了 $\log m$，如果边比点多很多，时间复杂度不会变，因为边的个数 $|E|$ 最多是 $|V|^2$，求对数之后，最多多了常量系数 2，也就是说性能最差会慢一倍。
...
## 最小生成树
给定一个连通的无向图 $G=(V,E)$，每条边 $e\in E$ 都有一个权重 $w_e$，最小生成树（`Minimum Spanning Tree`, `MST`）问题是找到 $G$ 的一个生成树（`spanning tree`），使得该树的权重之和最小。生成树是一个子图，包含 $G$ 的所有顶点，并且是一个树，也就是说 $T\subseteq E$。
...
while !H.empty()
    ...
    for Edge(v, w_min) in w_min.AdjacencyEdge()
        if v in X
            continue
        if weight_{vw_min} < key[v]
            key[v] = weight_{vw_min}
            winner[v] = (v, w_min)
            Delete v from H
            H.Push(v)
```
...
和 Dijkstra 算法类似，把上面的话再说一次：堆里面放的是待处理的数据这一点是确定的，但是如何放是可以选择的，一种常见方法就是将所有顶点都放到堆里面，起点的距离是 0，其余点的距离是最大值，即上面的伪代码阐述的方式。另一种方式是堆里面只放已经被发现但是还没有被处理的顶点。初始的时候只放起点，然后再处理邻接点的时候（发现）再放进堆。
...
给定一个图 $G=(V,E)$，每条边有一个权重 $w_e$，一条路径 $P$ 的瓶颈（`bottleneck`）是指路径上权重最大的边。如果边 $(v,w)\in E$ 是 $v-w$ 路径上最小的瓶颈，那么其满足最小瓶颈性质。

下面证明 Prim 算法选择的边满足最小瓶颈性质。假设 Prim 算法选择的边是 $(v,w)$，其中 $v\in X$，$w\in V-X$。那么根据算法有
$$c_{vw}\leq c_{xy}$$
其中 $x\in X,y\in V-X$。考虑任意一条 $v-w$ 的路径 $P$，由于 $v\in X$，$w\in V-X$，那么 $P$ 的瓶颈至少是 $c_{xy}$，又由于 $P$ 是任意的 $v-w$ 路径，那么 $P$ 的最小瓶颈是 $c_{vw}$，因此 Prim 算法选择的边满足最小瓶颈性质。

下面证明，在每条边的权重都不同的情况下，如果 $T$ 是 $G$ 的一个生成树，并且 $T$ 满足最小瓶颈性质，那么 $T$ 就是 $G$ 的最小生成树。

想象从一个空的图（只有顶点没有边）开始，我们向图中添加边，有的时候这条边连接了一个连通分量内部的两个顶点，即 $v,w$ 属于 $G$ 的同一个连通分量，那么存在一条 $v-w$ 的路径 $P$，添加边 $(v,w)$ 后会形成一个环，对连通分量的数量没有影响，我们称之为 C-类型的边（`C-type edge`）；有的时候这条边将两个连通分量连接起来，即 $v,w$ 分别属于 $S_1,S_2$ 两个连通分量，现在成了一个连通分量 $S_1\cup S_2$，我们称之为 F-类型的边（`F-type edge`），同时这条新增的边并不会形成环，因为假定有一个环 $C$，那么去掉新加的边 $(v,w)$ 得到的路径 $C-\{(v,w)\}$ 连接了点 $v,w$，说明这两个点在同一个连通分量，与前面的假设矛盾。

$T$ 是 $G$ 的一个生成树，没有环，因此从只有顶点的图开始，每次添加的边都是 F-类型的边。一开始有 $n$ 个顶点，即 $n$ 个连通分量，每添加一条 F-类型的边，连通分量的数量就会减少一个，因此添加 $n-1$ 条 F-类型的边后，所有的顶点就会连接成一个连通分量，也就是 $T$。因此 $T$ 中有 $n-1$ 条边。反之，如果加了 C-类型的边，那么就会形成环，连通分量的数量至少是 2，因此不可能是生成树。Prim 算法每次从 $X$ 中选择一个点，从 $V-X$ 中选择一个点，这两个点分别属于两个不同的连通分量，因此 Prim 算法每次选择的边都是 F-类型的边，所以 Prim 算法的输出是一个生成树。

有了上述铺垫，我们用反证法来证明：如果 $T$ 是满足最小瓶颈性质的生成树，那么 $T$ 就是最小生成树。假设 $T$ 不是最小生成树，那么存在一个生成树 $T^*$ 比 $T$ 的权重更小。由于两个树都是生成树，那么有 $n-1$ 条边，$T$ 至少包含一条边 $e_1=(v,w)$ 不在 $T^*$ 中。把这条边加到 $T^*$ 中会形成一个环。在 $T$ 中，边 $e_1$ 的权重是 $T$ 中的最小瓶颈，那么环中其他边的权重都不小于 $e_1$ 的权重，因此在 $T^*$ 中至少有一条边 $e_2$ 的权重不小于 $e_1$ 的权重。把 $e_2$ 从 $T^*$ 中去掉，那么得到的树 $T'=(T^*-\{e_2\})\cup \{e_1\}$ 的权重不大于 $T^*$ 的权重，因此也比 $T$ 的权重更小。$T^*$ 有 $n-1$ 条边且是连通的，$v-w$ 上我们去掉了 $e_2$，因此 $T^*-\{e_2\}$ 中 $v,w$ 不连通，那么 $T'=(T^*-\{e_2\})\cup \{e_1\}$ 中 $v,w$ 连通，因此 $T'$ 是连通的，一增一减，有 $n-1$ 条边，因此 $T'$ 是一个生成树。所以我们找到了权重更小的生成树 $T'$，与假设矛盾，因此 $T$ 就是最小生成树。
...
使用 `parent tree` 来实现并查集，`parent[i]` 表示元素 $i$ 的父节点，如果 $i$ 是一个集合的根节点，那么 `parent[i]` 就是 $i$ 本身。初始化时，每个元素都是一个单独的集合，因此 `parent[i]=i`。`find` 操作就是沿着 `parent` 数组向上查找，直到找到一个元素 $i$ 满足 `parent[i]=i`，这个元素就是所在集合的标识符。这里可以使用隔代压缩技术，在查找的时候使当前路径上的节点的深度减半；或者使用递归方式的完全压缩技术，在查找的时候将当前路径上的所有节点直接连接到根节点上，但是这样树很深的话会导致移出。因此我的实现采用了前者。`union` 操作就是将两个元素所在集合的根节点连接起来，即将其中一个根节点的 `parent` 指向另一个根节点。为了让树的深度尽可能小，额外使用一个数字 `size[i]` 来记录以 $i$ 为根节点的集合的大小，在合并的时候将较小的集合连接到较大的集合上。这样整个树的深度就不会超过 $\log n$，因此 `find` 和 `union` 的时间复杂度都是 $O(\log n)$。
...
有了并查集，我们可以高效地判断两条边的两个端点是否在同一个集合中，即是否已经连通了，那么可以优化 Kruskal 算法。
```
T={}
sort(E)
UF = new UnionFind(n)
...
return T
```
首先对边按照权重进行排序，时间复杂度是 $O(m\log m)$，由于 $m$ 最多是 $n^2$，因此排序的时间复杂度也可以简化为 $O(m\log n)$。然后循环迭代 $m$ 次，每次迭代两次 `find`，时间复杂度是 $mO(\log n)$。$n-1$ 次 `union`，时间复杂度是 $nO(\log n)$，因此总的时间复杂度是 $O((m+n)\log n)$。
...
首先，在截止处理到边 $e=(v,w)$ 的迭代，生成树 $T$ 中 $v,w$ 属于同一个连通分量。假定边 $e$ 是 C-类型的边，那么 $v,w$ 已经在同一个连通分量了；假定边 $e$ 是 F-类型的边，那么 $v,w$ 不在同一个连通分量，当前迭代就会把 $e$ 加入到 $T$ 中，那么 $v,w$ 就在同一个连通分量。
