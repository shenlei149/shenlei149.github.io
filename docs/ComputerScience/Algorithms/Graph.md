...
想象从一个空的图（只有顶点没有边）开始，我们向图中添加边，有的时候这条边连接了一个连通分量内部的两个顶点，即 $v,w$ 属于 $G$ 的同一个连通分量，那么存在一条 $v-w$ 的路径 $P$，添加边 $(v,w)$ 后会行程一个环，对连通分量的数量没有影响，我们称之为 C-类型的边（`C-type edge`）；有的时候这条边将两个连通分量连接起来，即 $v,w$ 分别属于 $S_1,S_2$ 两个连通分量，现在成了一个连通分量 $S_1\cup S_2$，我们称之为 F-类型的边（`F-type edge`），同时这条新增的边并不会形成环，因为假定有一个环 $C$，那么去掉新加的边 $(v,w) 得到的路径 $C-\{(v,w)\}$ 连接了点 $v,w$，说明这两个点在同一个连通分量，与前面的假设矛盾。
...
有了上述铺垫，我们用反证法来证明 $T$ 是满足最小瓶颈性质的生成树，那么 $T$ 就是最小生成树。假设 $T$ 不是最小生成树，那么存在一个生成树 $T^*$ 比 $T$ 的权重更小。由于两个树都是生成树，那么有 $n-1$ 条边，$T$ 至少包含一条边 $e_1=(v,w)$ 不在 $T^*$ 中。把这条边加到 $T^*$ 中会形成一个环。在 $T$ 中，边 $e_1$ 的权重是 $T$ 中的最小瓶颈，那么环中其他边的权重都不小于 $e_1$ 的权重，因此在 $T^*$ 中至少有一条边 $e_2$ 的权重不小于 $e_1$ 的权重。把 $e_2$ 从 $T^*$ 中去掉，那么得到的树 $T'=(T^*-\{e_2\})\cup \{e_1\}$ 的权重不大于 $T^*$ 的权重，因此也比 $T$ 的权重更小。$T^*$ 有 $n-1$ 条边且是连通的，$v-w$ 上我们去掉了 $e_2$，因此 $T^*-\{e_2\}$ 中 $v,w$ 不连通，那么 $T'=(T^*-\{e_2\})\cup \{e_1\}$ 中 $v,w$ 连通，因此 $T'$ 是是连通的，一增一减，有 $n-1$ 条边，因此 $T'$ 是一个生成树。所以我们找到了权重更小的生成树 $T'$，与假设矛盾，因此 $T$ 就是最小生成树。
...
return T
```
最朴素的实现的实现复杂度是 $O(nm)$，其中 $n$ 是点的个数，$m$ 是边的个数。因为上面的循环需要 $m$ 次，每次中需要使用 BFS 或 DFS 来判断 $(v,w)$ 是否已经连通了。下面介绍使用并查集（`Union-Find`）来加速这个过程，时间复杂度可以降到 $O((m+n)\log n)$，这里也假设图是连通的，那么时间复杂度可以简化为 $O(m\log n)$。
...
有了并查集，我们可以高效的判断两条边的两个端点是否在同一个集合中，即是否已经连通了，那么可以优化 Kruskal 算法。
```
T={}
sort(E)
UF = new UnionFind(n)
