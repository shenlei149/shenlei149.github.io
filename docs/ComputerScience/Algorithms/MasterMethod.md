...
二分查找只有一次递归调用，$a=1$，每次规模缩小一倍，因此 $b=2$。除了递归之外，仅需要比较 `key` 和中间值，时间复杂度为 $O(1)$，因此 $d=0$，即 $a=1=2^0=b^d$，那么时间复杂度是 $O(\log n)$。
...
直观地看，$a<b^d$ 时，子问题增加的速率小于工作量减少的比率，也就是根节点工作量最大，$n^d$ 项的权重更大。这与第三种情况恰好相反，叶子节点权重最大，而叶子节点位于第 $\log_b n$ 层，时间复杂度为 1 的叶子节点个数是 $a^{\log_b n}$，这也就是总复杂度。
