...
假定第一轮中位数分别是 $x_1,x_2,\cdots,x_k$，这里 $k=\frac{n}{5}$，中位数的中位数是 $x_{k/2}$，这个中位数在第二轮中，比 $k/2=n/10$ 的数大，在那些 $x_i,i<=k/2$ 的组中，还有两个元素肯定也比中位数的中位数要小，因此这个值比 $3n/10$ 个元素要大。反过来，这个值比 $3n/10$ 个元素要小。也就是说，这个中位数的中位数一定处于 30 分位和 70 分位之间。也就是说说，第二次递归的数组规模最大是 $\frac{7n}{10}$
