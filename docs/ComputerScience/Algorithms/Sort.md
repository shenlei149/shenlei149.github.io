...
前面两种情况容易处理，递归调用即可。如何计算第三种情况的逆序对个数呢？下面分析归并两个有序数组和逆序对的关系。

假定两个有序数组 $A, B$，长度均为 $n$。归并时，比较 $A$ 的第 $i$ 个值和 $B$ 的第 $j$ 个值，如果 $A$ 的第 $i$ 个值较小，就将其放入结果数组，这里没有逆序对；如果 $A$ 的第 $i$ 个值较大，那么包含该值在内的 $A$ 中剩余元素都比 $B$ 的第 $j$ 个值大，因此有 $n-i+1$ 个逆序对，即 $A$ 中尚未归并的元素个数。
...
整个过程如上图所示。对于归并函数而言，假定两个数组的长度都是 $l$，那么开销是 $O(l)$。假定第 $i$ 层有 $2^i$ 个子数组，每个数组长度是 $n/2^i$，其中 $n$ 是原始数组的大小，那么每层合并的开销是 $O(n)$。整棵树的高度是 $\log_2 n+1$，因此归并排序的时间复杂度是 $O(n\log n)$。
...
上面快排的实现中使用第一个元素作为枢轴元素在这种场景下是不适用的，必须采用随机化选择枢轴元素的方法。因为如果递归数组的后半部分，枢轴元素是第一个，此时调用划分方法，枢轴元素不动，会永远递归下去。另外，如果数组中包含重复元素并且第 $i$ 个就是重复，对于这种情况也要小心的进行额外处理才能避免无限递归，换句话说，像上面快排划分的代码仅传入小于比较器是不够的，需要知道什么时候相等，然后再处理。
...
下面证明期望运行时间复杂度是 $O(n)$。一个思路是与快排类似，不过这里有另一种更简单的证明方式。

与之前类似，递归调用之外的划分复杂度是 $O(n)$，准确的说，最多有 $cn$ 次操作。我们这里假定原始数据的大小是 $n$，然后整个递归过程分成若干个阶段，第 $j$ 个要处理的元素数 $n_j$ 满足
$$(\frac{3}{4})^{j}n>n_j>(\frac{3}{4})^{j+1}n$$
那么第 $j$ 个阶段的操作数最大是
$$cn(\frac{3}{4})^{j}$$
使用随机变量 $X_j$ 表示第 $j$ 个阶段递归调用的次数。那么算法运行时间的上界是
$$cn\sum_{j\geq 0}(\frac{3}{4})^jX_j$$
其期望值是
$$E\leq cn\sum_{j\geq 0}(\frac{3}{4})^jE[X_j]$$
...
这个过程有点类似扔硬币直到扔到正面的过程。不过稍有不同。这里令 $N$ 是连续扔硬币出现正面的次数，那么 $E[X_j]\leq E[N]$。因为（1）阶段 $j$ 可以被略过，但是扔硬币的过程至少要扔一次；（2）硬币为正面的概率固定是 50%，但是选择枢轴元素的概率至少是 50%。

硬币场景是参数为 1/2 的几何随机变量，期望值是 2。也可以如下理解整个过程，至少扔一次，可能是正面，整个过程结束，如果是反面（一半的概率），那么递归这个过程。
$$E[N]=1+\frac{1}{2}E[N]$$
因此
$$E[X_j]\leq 2$$
将其代入上面的方程有
$$E\leq cn\sum_{j\geq 0}(\frac{3}{4})^jE[X_j]=2cn\sum_{j\geq 0}(\frac{3}{4})^j=8cn$$
因此运行时间的期望值是 $O(n)$。
...
![](./Figures/MedianOfMedians.png)

假定数组有 $n$ 个元素，求第 $i$ 大的元素。整个算法的过程如下：

1. 将数组分成五个一组，求出每组的中位数，组成一个 $\frac{n}{5}$ 个元素的数组。
2. 递归调用算法，求中位数。这里是要求第 $\frac{n}{10}$ 个元素。
3. 使用上一步得到的中位数的中位数作为枢轴元素将数组划分成两个部分，并返回索引 $j$。
4. 如果 $j$ 恰好是 $i$，那么返回。
5. 如果 $j>i$，那么在前一半数组找；如果 $j<i$，那么在后一半查找，并且找第 $i-j$ 个元素。

算法最差情况下时间复杂度是 $O(n)$。虽然随机化算法最差是 $O(n^2)$，但是平均是 $O(n)$，实践中基本都是后者的情况，同时复杂度的常量系数更小，因此随机化算法更实用。

下面证明这个算法的时间复杂度是 $O(n)$。

首先，递归之外有两部分。第一部分是计算得到 $\frac{n}{5}$ 个子数组的中位数，每个子数组只有 5 个数，排序再取第三个即可，每个子数组处理时间复杂度是 $O(1)$，那么这部分开销是 $O(n)$。第二部分是划分，这个在快排算法中分析过，复杂度是 $O(n)$，因此递归外的时间复杂度是 $O(n)$。

递归有两次。第一次是传入了一个大小为 $\frac{n}{5}$ 的数组，因此复杂度可以表示为 $T(\frac{n}{5})$。第二次是划分之后，传入了一个子数组，大小未知，下面分析其最大的可能值。

假定第一轮中位数分别是 $x_1,x_2,\cdots,x_k$，这里 $k=\frac{n}{5}$，中位数的中位数是 $x_{k/2}$。这个中位数在第二轮中，比 $k/2=n/10$ 个数大；在那些 $x_i,i<=k/2$ 的组中，还有两个元素肯定也比中位数的中位数要小，因此这个值比 $3n/10$ 个元素要大。反过来，这个值比 $3n/10$ 个元素要小。也就是说，这个中位数的中位数一定处于 30 分位和 70 分位之间。也就是说，第二次递归的数组规模最大是 $\frac{7n}{10}$

![](./Figures/30-70.png)

这个算法的核心就在于 $\frac{1}{5}+\frac{7}{10}=\frac{9}{10}<1$。

那么整个算法复杂度的递归公式是
$$T(n)\leq T(\frac{n}{5})+T(\frac{7n}{10})+cn$$
这里有两个递归项，没法使用主方法，下面使用猜测检验法来证明其时间复杂度是 $O(n)$。

假定算法复杂度是 $O(n)$，那么 $T(n)\leq ln$，这里我们取 $l=10c$，那么 $T(n)\leq 10cn$。当 $n=1$ 时，$T(1)\leq 10c$，是 1 的常量倍，假设成立。根据递归法，假设 $k=1,2,3,\cdots,n-1$ 时都有 $T(k)\leq 10ck$，现在考虑 $n$ 时
$$\begin{aligned}
T(n)&\leq T(\frac{n}{5})+T(\frac{7n}{10})+cn\\
&\leq \frac{ln}{5}+\frac{7ln}{10}+cn\\
&=2cn+7cn+cn\\
&=10cn\\
&=ln
\end{aligned}$$
