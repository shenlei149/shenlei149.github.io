...
首先考虑两种极端情况。第一种是所有任务的长度都相同，假定长度是 1，那么它们的完成时间是 $\{1, 2, \ldots, n\}$，很明显，应优先处理权重大的任务，总时间更短。第二种是所有任务的权重都相同，第一个任务完成所需的时间会对所有任务的完成时间产生影响，因此应该先处理长度短的任务。由此可以得到一个启发：应该先处理权重大的任务，或者先处理长度短的任务。
...
### Huffman 编码
首先介绍前缀码（`prefix-free code`）：任意两个符号的编码都不互为前缀。定长编码，比如 ASCII 编码，通常是前缀码。再比如对于 $\Sigma=\{A, B, C, D\}$，如果我们使用定长编码，那么每个符号的编码长度都是 2，例如 $A=00, B=01, C=10, D=11$，这是前缀码。如果我们使用变长编码，例如 $A=0, B=10, C=110, D=111$，也是前缀码，任意两个编码都不互为前缀。前缀码的好处是编码没有歧义，解码简单明了。如果各个符号出现的频次不同，那么变长前缀码比固定长度编码更高效。
...
下面证明存在一个最优的树，其中 $a, b$ 是兄弟节点。不失一般性，假定 $T$ 是最优的树，树中的节点要么是叶子节点，要么是内部节点（有两个子节点）。假定 $x,y$ 是最深的、具有公共父节点的两个叶子节点，分别位于左右子树。交换 $a,x$ 和 $b,y$，得到新的树 $T*$，$T*$ 中 $a,b$ 是兄弟节点。对于 $T$ 和 $T*$ 中除 $a,b,x,y$ 外的叶子节点，它们的深度和权重都相同，那么
$$\begin{aligned}
