...
为了快速实现选择和秩操作，可以在每个节点中维护一个额外的属性 `size`，表示以该节点为根的子树中的节点数量。这样选择操作可以通过比较 `i` 与左子树的大小来决定是继续在左子树中选择还是在右子树中选择。假定左子树的 `size` 是 `j`，如果 `i` 等于 `j`，那么当前节点就是第 `i` 小的节点；如果 `i` 小于 `j`，那么继续在左子树中选择；如果 `i` 大于 `j`，那么继续在右子树中选择，并且将 `i` 更新为 `i - j - 1`。这里的 `i ` 从 0 开始计数。秩操作的实现与选择操作类似，首先比较 `key` 与当前节点的键值，如果相等则返回左子树的大小；如果 `key` 小于当前节点的键值，则继续在左子树中计算秩；如果 `key` 大于当前节点的键值，则继续在右子树中计算秩，并且将结果加上左子树的大小加 1，因为当前节点也比 `key` 小。
