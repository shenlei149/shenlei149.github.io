...
## 整数的运算
### 无符号加法
两个非负整数 $x,y$，范围是 $0\leq x,y<2^w$，和的范围值 $0\leq x+y\leq 2^{w+1}-2$，表示这个数需要 $w+1$ 位。如果想要表示两个 $w+1$ 位非负整数的和，那么需要 $w+2$ 位。随着字长的增加，如果想要完美表示算术结果，就不能给字长设定边界。大部分的编程语言只支持固定大小的算数，因此加法和乘法和数学中对应的算术运算不一样。
