...
向量算术指令往往只允许一个向量的 `n` 个元素与另一个向量的 `n` 个元素参与运算。这大大简化了构建平行的向量单元，即多个向量通路（`vector lane`）组成一个向量单元。和高速公路一样，我们可以添加更多的向量通路来提升向量单元的峰值吞吐。如下图所示，四道并行，那么处理时间会缩短四倍。为了让这种多通路结构有效，应用和架构都必须支持更长的向量，
