...
* 引用完整性（`referential integrity`）：这一点是希望确保出现在一个关系（表）的某个属性（列）的值一定要在另一个关系的某个属性中。比如，表 course 中 dept_name 列的值必须在 表 department 的 dept_name 里面。当违反参考完整性约束时，会拒绝数据的插入。
