...
我们可以将软件开发分成两个层次，设计层（`design level`）包含架构和设计，代码层（`code level`）包含实现细节（`implementation details`）。实现细节这一层有一些常见的实现模式（`implementation patterns`），比如 RAII、Pimpl、SFINAE（`Substitution Failure Is Not An Error`）等等，这些模式是 C++ 特有的，其他语言可能没有或者有不同的实现方式。再往上是软件设计，这一层包含一些设计模式（`design patterns`），比如装饰者、策略等等，这些就是语言无关的了。再往上是软件架构，如前所述，这里的共识不多，通常涉及关键决策。这一层包含一些架构模式（`architectural patterns`），比如微服务、客户端-服务器等。两者的共同点都是解决整体结构和相互依赖的问题。架构是更高层次的设计，解决的是模块、组件级别的，而设计则是更细粒度的，解决的是类、函数级别的。

惯用法（`idiom`）的出现使得实现和设计两者的界限更加模糊一些。惯用法表示某种模式（`pattern`），在某种语言中被广泛使用，比如前面提到的 RAII 就是 C++ 中的一种惯用法，还有 copy-and-swap 也是 C++ 中的一种惯用法。大部分的 C++ 惯用法都是实现层次的，设计层次的惯用法相对较少一些，比如 NVI（`Non-Virtual Interface`）和 Pimpl 都是一种设计层次的惯用法，分别脱胎于设计模式中的模板方法（`Template Method`）和桥接模式（`Bridge Pattern`）。这里涉及到一个问题，就是掌握编程语言的细节和掌握软件设计的原则哪个更重要？答案是两者都重要。没有必要厚此薄彼。
...
```
上面这个基类的定义看似很好，能够导出 JSON 这种通用格式，也能将数据序列化为二进制并写入 `ByteStream`，可能用于 Word、PDF 等文档。但是这个类引入了三类依赖。

第一类是 `ExportToJSON` 方法引入了对 JSON 库的依赖，这个库可能是第三方的，也可能是我们自己写的。为了一致性，所有的子类不得不依赖同一个类库，因此这些子类并没有真正独立，而是耦合到了一起。同时，由于依赖了 JSON 类库，类本身变得不够轻量，后续重用也是一个问题。`Serialize` 很可能会依赖于 `protobuf` 这种二进制序列化类库，这一依赖与 JSON 类库正交，但可能会引发连锁反应。
...
SRP 的思想就是分离这些关注点，将由不同理由导致变化的部分隔离到不同的变化点（`variation point`）。比如将 JSON 和序列化的功能抽取到不同的类中，这样就能够将这些变化点隔离开来，降低耦合，提高内聚。下面是一种可能的依赖关系。后续章节会介绍一些设计模式，以及如何实现这些分离的设计。
```
...
SRP 告诉我们每个事物只能有一个职责（一个变化的理由），DRY 原则告诉我们每个职责也应该有且只有一份实现。该原则建议不要在多处重复同样的信息，而是要通过合理的设计，确保只需要在一个地方做出修改就足够了。理想情况下，税率这个信息应该只在一处定义即可。通常情况下，DRY 原则和 SRP 是相辅相成的，SRP 能够帮助我们更好地实现 DRY 原则，反之亦然。不过在一些特殊的情况下，要想同时满足这两个原则需要一些额外的设计。后续章节会介绍一些设计模式，阐述如何实现这些原则。
...
```
这保证了约束的最小化：`std::input_iterator` 支持读取和单次遍历，输出迭代器只需支持写入和递增。若错误地用要求更强的 `std::forward_iterator` 约束输入迭代器，就会引入不必要的多次遍历要求，导致某些场景无法使用这个 `copy` 函数，例如输入流迭代器不满足多次遍历的要求。
...
```
你可能会说，或许有 `AddBlob()` 这个 `public` 函数会调用 `UpdateCollection()`，我们可以通过测试 `AddBlob()` 来间接测试 `UpdateCollection()`。如果 `UpdateCollection()` 只是 `AddBlob()` 的实现细节，那么这正是合适的黑盒测试（`black-box test`），只测试对外可观察的预期行为，而不依赖其内部是否调用了某个特定函数。若实现不再调用 `UpdateCollection()`，相关测试也不应约束这一已经改变的实现细节。
...
你可能会说，把这个函数修改成 `protected`，让测试类继承 `Widget` 类。这通常会引入不必要的继承耦合，并不推荐。

仅为测试私有函数而将其公开或抽取出来并不是正确的解法。只有当 `UpdateCollection()` 包含可独立描述、具有独立变化原因或可复用的行为时，才应将其抽取为类外函数，或者独立类的成员函数，此时可以直接测试其公开行为，也不会引入不必要的测试依赖。
```cpp
...
```
这样将原本放在一起的东西分离开，是否违背了 SRP 呢？不一定。仅仅因为一个函数需要测试，并不能说明它必须是独立单元，或说明原有类违反了 SRP。只有当它代表独立的概念、能够单独演进时，抽取才有助于让 `Widget` 聚焦自身职责，并使新单元的行为能够被直接测试。

这样是否违反了封装呢？不一定。Scott Meyers 在《Effective C++》中提到过，在不需要访问私有成员时，应优先考虑非成员、非友元函数（`non-member, non-friend function`）。成员函数天生能访问所有数据成员，容易访问到不相关的数据成员，导致耦合度增加。抽取后的 `UpdateCollection()` 若只接收其真正需要的输入，便可缩小其可访问的状态范围，改善访问控制和内聚。

在适合抽取的情况下，上述设计可带来以下好处：（1）能够直接测试 `UpdateCollection()` 的公开行为；（2）缩小可访问状态的范围；（3）使 `UpdateCollection()` 与 `Widget` 解耦，并可能复用于其他类。

这里并不是鼓励把所有函数都设计成独立函数，甚至单独抽取到一个类。我们应仔细分析被放入 `private` 的函数是否确实具有独立职责和可独立验证的行为。如果这个函数是虚函数该怎么办呢？后续章节会讨论这个问题。
...
```
在序列化的时候，可能第一个字节就是这个 `DocumentType`，用来区分不同类型的文档。这样 `PDF` 类会依赖包含 `Word` 枚举值的 `DocumentType`，从而产生不必要的耦合。整个结构如下所示。
```
                   Document
                      ^
                      |
...
```
如果我们想要添加一个子类，比如 `XML`，我们就需要修改 `DocumentType` 枚举。这至少会导致依赖该枚举的 `PDF` 和 `Word` 重新编译。更糟糕的是，这限制了其他人对代码进行扩展的能力，因为这可能是一个类库，而很多人没有权限修改 `DocumentType` 枚举。从设计的角度来说，`PDF` 和 `Word` 这两个类本不应该知道新增文档类型 `XML` 的存在。
...
### 编译期扩展性
日常开发中，常常会使用 C++ 标准库，它的设计考虑了扩展性，因此我们可以轻松地使用它来实现自己的功能。先看第一个例子 `std::swap`，它的实现如下所示。
```cpp
template<class T>
void swap(T &a, T &b)
{
...
### 避免过早进行扩展性设计
扩展性很重要，但是这不意味着应该不加思考地在实现细节层面广泛使用多态、模板来确保未来的扩展性。和不要过早地优化程序、过早地进行关注点分离一样，我们也不应该过早地关注扩展性。如果还不知道哪些地方会如何演进，那么就等到需要扩展的时候再进行设计和实现就好，不要预判一个可能永远不会扩展的点。未来的需求会指导我们，给出未来演进的方向，此时再重构代码，使得后续更容易扩展。
