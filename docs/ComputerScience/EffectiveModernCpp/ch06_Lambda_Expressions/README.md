...
闭包类（`closure class`）是闭包实例化的类。每一个 lambda 使得编译器生成唯一的闭包类。lambda 内的语句是对应闭包类的成员函数的可执行指令。
