...
### 后向差分公式
另一个多步法是使用多项式 $P_k(t)$ 近似初值问题 $(1)$ 中的 $\phi(t)$ 而不是它的导数 $\phi'(t)$。然后对 $P_k(t)$ 求导，令 $P_k'(t_{n+1})=f(t_{n+1},y_{n+1})$ 得到 $y_{n+1}$ 的隐式公式。这称为后向差分公式（`backward differentiation formula`）。

最简单的例子是一阶多项式 $P_1(t)=At+B$，那么要满足 $P_1(t_n)=y_n,P_1(t_{n+1})=y_{n+1}$，那么 $A,B$ 要满足
$$\begin{aligned}
At_n+B&=y_n\\
At_{n+1}+B&=y_{n+1}
\end{aligned}\tag{12}$$
求解得到
$$A=\frac{y_{n+1}-y_n}{h},B=\frac{y_{n+1}t_n-y_nt_{n+1}}{h}\tag{13}$$
由于 $P_1'(t)=A$，又因为 $P_1'(t_{n+1})=f(t_{n+1},y_{n+1})$，因此 $A=f(t_{n+1},y_{n+1})$。结合 $(13)$ 就得到了一阶后向差分公式（`first-order backward differentiation formula`）
$$y_{n+1}=y_n+hf(t_{n+1},y_{n+1})\tag{14}$$
这就是在 8.1 小节讨论的后向欧拉公式。

使用更高阶的多项式和相应更多的点，可以得到任意阶后向差分公式。下面是二阶后向差分公式（`second-order backward differentiation formula`）
$$y_{n+1}=\frac{1}{3}(4y_n-y_{n-1}+2hf(t_{n+1},y_{n+1}))\tag{15}$$
下面是四阶后向差分公式（`fourth-order backward differentiation formula`）
$$y_{n+1}=\frac{1}{25}(48y_n-36y_{n-1}+16y_{n-2}-3y_{n-3}+12hf(t_{n+1},y_{n+1}))\tag{16}$$
这两个公式的局部截断误差分别正比于 $h^3,h^5$。

例 2 使用步长 $h=0.1$ 的四阶后向差分公式来估算例 1 中在 $t=0.4$ 时解 $y=\phi(t)$ 的近似值。

解：$y_0,\cdots,y_3$ 在例 1 中已经给出，$n=3,h=0.1$，都代入 $(16)$ 得到
$$y_4=4.6837842+0.192y_4$$
因此
$$y_4=5.7967626$$
和精确值 $\phi(t)=5.7942260$ 相比，误差是 0.0025366。这比四阶亚当斯-巴什弗思的结果要好，但是不如四阶预测-修正法，远不如亚当斯-莫尔顿法。

比较单步法与多步法时，要综合考虑多个因素。四阶龙格-库塔法在每一步都需要求四次 $f$ 函数的值；而四阶亚当斯-巴什弗思法（除了开始阶段）每步仅需一次求值，预测-修正法需要两次。因此，在给定步长 $h$ 的情况下，后两种方法的运行速度很可能比龙格-库塔法快得多。不过，如果龙格-库塔法的精度更高，并因此可以采用更大的步长，更少的步数，那么这种速度上的差异就会缩小，甚至可能会消失。
