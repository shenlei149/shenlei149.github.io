...
$$y(t_0)=y_0\tag{2}$$
假定 $f,f_y$ 在平面 $ty$ 内某个包含 $(t_0,y_0)$ 的矩形内是连续的，根据定理 2.4.2，在包含 $t_0$ 的某个区间内有唯一解 $y=\phi(t)$。如果方程 $(1)$ 是非线性的，解存在的区间比较难确定，或许无法表达成 $f$ 的某种简单关系。不过这里假定要讨论的各种情况下，在研究区间内，初始问题 $(1),(2)$ 存在唯一解。
...
### 数值近似的误差
使用类似欧拉公式的数值方法来求解初值问题，在接受近似数值解之前需要回答一系列的问题。这些问题之一是收敛（`convergence`）。随着步长 $h$ 趋于零，数值近似 $y_1,y_2,\cdots,y_n$ 是否趋于真实解的值？如果答案是肯定的，下一个问题是收敛的速度如何？我们想要找到充分小但是不是太小的步长来确保达到需要的精度。一个不必要的太小的步长会使得计算步骤变多，甚至会导致精度损失。

初值问题的数值近似解的误差主要来源于三个方面。

1. 用于计算的公式带来的误差。比如欧拉公式使用直线来近似真实的解。
2. 对于每一步，使用近似值 $y_n$ 来计算下一个近似值 $y_{n+1}$ 带来的误差。
3. 计算机计算只有有限精度。

首先假定计算机可以执行任意精度，也就是说每一步可以保留无限精度。那么初值问题 $(1),(2)$ 的解 $y=\phi(t)$ 和数值近似解 $y_n$ 之间的误差可以表示为 $E_n$
$$E_n=\phi(t_n)-y_n\tag{14}$$
误差 $E_n$ 称为全局截断误差（`global truncation error`），主要是由于前面两种原因引入的，即公式和输入数据带来的误差。

不过，现实中要考虑计算过程中使用有限精度算术带来的误差，这是每一步只保留有限数字带来的误差。这称为舍入误差（`round-off error`）$R_n$
$$R_n=y_n-Y_n\tag{15}$$
其中 $Y_n$ 是实际计算得到的结果。

计算 $\phi(t_n)$ 时的误差的绝对值是
$$|\phi(t_n)-Y_n|=|\phi(t_n)-y_n+y_n-Y_n|\tag{16}$$
利用三角不等式 $|a+b|\leq |a|+|b|$ 得到
$$\begin{aligned}
|\phi(t_n)-Y_n|&\leq|\phi(t_n)-y_n|+|y_n-Y_n|\\
&=|E_n|+|R_n|
\end{aligned}\tag{17}$$
总误差的上界是全局截断误差和舍入误差的绝对值之和。

对于本书讨论的数值方法，可以分析估计全局截断误差。分析舍入误差更困难，这依赖于依赖于数值类型、计算的步骤、舍入的方法等等。详尽的舍入误差分析超出了本书的范围。

单独分析全局截断误差中由于近似公式带来的误差对分析问题很有帮助。可以通过假定第 $n$ 步的输入是精确的，即 $y_n=\phi(t_n)$。这个误差称为局部截断误差，用 $e_n$ 表示。

### 欧拉法的局部截断误差
假定初值问题 $(1),(2)$ 的解 $y=\phi(t)$ 在研究区间上有连续的二阶导。为了保证这一点，需要假定 $f,f_t,f_y$ 是连续的。因为如果 $f$ 满足这个属性并且 $\phi$ 是初值问题 $(1),(2)$ 的解，那么
$$\phi'(t)=f(t,\phi(t))$$
根据链式法则
$$\begin{aligned}
\phi''(t)&=f_t(t,\phi(t))+f_y(t,\phi(t))\phi'(t)\\
&=f_t(t,\phi(t))+f_y(t,\phi(t))f(t,\phi(t))
\end{aligned}\tag{18}$$
右边都是连续函数，因此 $\phi''$ 也是连续的。

在 $t_n$ 处将 $\phi$ 展开泰勒级数并保留余项得到
$$\phi(t_n+h)=\phi(t_n)+\phi'(t_n)h+\frac{1}{2}\phi''(\overline{t_n})h^2\tag{19}$$
其中 $t_n<\overline{t_n}<t_n+h$。注意到 $\phi(t_n+h)=\phi(t_{n+1}),\phi'(t_n)=f(t_n,\phi(t_n))$，那么 $(19)$ 可以写作
$$\phi(t_{n+1})=\phi(t_n)+hf(t_n,\phi(t_n))+\frac{1}{2}\phi''(\overline{t_n})h^2\tag{20}$$
假定我们已知 $t_n$ 时正确的值 $y_n=\phi(t_n)$，那么可以估算 $\phi(t_{n+1})$ 的值
$$y_{n+1}^*=\phi(t_n)+hf(t_n,\phi(t_n))\tag{21}$$
这里星号表示这是 $\phi(t_{n+1})$ 的近似值。$\phi(t_{n+1})$ 和 $y_{n+1}^*$ 的差值是 $n+1$ 步的局部截断误差，用 $e_{n+1}$ 表示。根据 $(20),(21)$ 可以得到
$$e_{n+1}=\phi(t_{n+1})-y_{n+1}^*=\frac{1}{2}\phi''(\overline{t_n})h^2\tag{22}$$
欧拉公式的局部截断误差与 $h$ 的平方成正比，与解 $\phi$ 的二阶导成正比。$(22)$ 说明误差依赖于 $n$，通常来说每一步是不同的。在区间 $[a,b]$ 成立的一致边界是
$$|e_n|\leq\frac{1}{2}Mh^2\tag{23}$$
其中 $M$ 是 $\phi''$ 在区间 $[a,b]$ 上的最大值。由于 $(23)$ 是最差情况，因此它是对真实局部截断误差的高估。

$(23)$ 的一个作用是估计使局部截断误差不超过某个指定值的步长。比如如果要求局部截断误差不超过 $\epsilon$，那么
$$\frac{1}{2}Mh^2\leq\epsilon,h\leq\sqrt{\frac{2\epsilon}{M}}\tag{24}$$
使用 $(22),(23),(24)$ 最大的困难是估计 $|\phi''(t)|$ 或 $M$。不过，这个式子主要想表达的是误差正比于 $h^2$。比如，$h$ 减少一倍，那么结果的误差是原来误差的四分之一。

相比局部截断误差，更重要的是全局截断误差 $E_n$。不过分析 $E_n$ 比要比分析 $e_n$ 困难的多。能够证明在有限区间内使用欧拉方法的全局截断误差不会超过 $h$ 的常数倍，那么
$$|E_k|\leq Kh\tag{25}$$
$K$ 是常数。欧拉法是一阶方法（`first-order method`），因为它的全局截断误差正比于步长的一次幂。

由于局部截断误差更容易分析，后续我们使用它来分析比较不同方法的精度。如果我们预先知道初值问题的解，那么可以用 $(22)$ 分析局部截断误差随 $t$ 增长的变化。
