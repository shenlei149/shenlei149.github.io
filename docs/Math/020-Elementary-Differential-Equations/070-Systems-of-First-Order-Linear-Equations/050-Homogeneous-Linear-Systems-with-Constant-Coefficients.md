...
例 2 和例 3 中 $2\times 2$ 的方程组的特征值是实数且不同。特征值符号相反或者相同。还有一个可能性是特征值是 0，不过这意味着 $\det\bold{A}=0$，不过这和本节开始的假定不符。

回到 $n\times n$ 的方程组 $(1)$，处理方式和之前的例子相同。为了求解，首先求解 $\bold{A}$ 的特征值和特征向量。特征值 $r_1,\cdots,r_n$ 是 $n$ 阶多项式方程组
$$\det(\bold{A}-r\bold{I})=0\tag{26}$$
的根，它们不必完全不同。特征值和特征方程的性质决定了方程组 $(1)$ 的性质。假定 $\bold{A}$ 是实数矩阵，需要考虑 $\bold{A}$ 的特征值是如下几种情况：

* 所有特征值是实数且都不相等。
* 一些特征值是共轭复数。
* 一些特征值是相等的，复数或者实数都适用。

如果 $n$ 个特征值都是实数且不相等，那么特征值的代数重数和几何重数相等。每一个特征值 $r_i$ 都关联一个特征向量 $\bold{\xi}_i$，$n$ 个特征向量 $\bold{\xi}^{(1)},\cdots,\bold{\xi}^{(1)}$ 都线性无关的。微分方程组 $(1)$ 的解是
$$\bold{x}^{(1)}(t)=\bold{\xi}^{(1)}e^{r_1t},\cdots,\bold{x}^{(n)}(t)=\bold{\xi}^{(n)}e^{r_nt}\tag{27}$$
为了证明这些解可以构成基础解析，计算其朗斯基
$$\begin{aligned}
W[\bold{x}^{(1)},\cdots,\bold{x}^{n}](t)&=\begin{vmatrix}
...
\end{vmatrix}
\end{aligned}\tag{28}$$
首先，指数函数不为零。齐次，特征向量 $\bold{\xi}^{(1)},\cdots,\bold{\xi}^{(n)}$ 是线性无关的，因此 $(28)$ 最后的行列式不为零，那么朗斯基 $W[\bold{x}^{(1)},\cdots,\bold{x}^{(n)}]$ 不为零。因此 $\bold{x}^{(1)},\cdots,\bold{x}^{(n)}$ 是基础解系，那么方程组 $(1)$ 的通解是
$$\bold{x}=c_1\bold{\xi}^{(1)}e^{r_1t}+\cdots+c_n\bold{\xi}^{(n)}e^{r_nt}\tag{29}$$
如果 $\bold{A}$ 是实数对称的，是埃尔米特矩阵，那么所有的特征值 $r_1,\cdots,r_n$ 都必须是实数，即使一些特征值重复了，也总能找到 $n$ 个特征向量 $\bold{\xi}^{(1)},\cdots,\bold{\xi}^{(n)}$ 是线性无关的，因此方正组 $(1)$ 的基础解系是 $(27)$，通解是 $(29)$。

例 4 求
$$\bold{x}'=\begin{pmatrix}
0&1&1\\1&0&1\\1&1&0
\end{pmatrix}\bold{x}\tag{30}$$
的通解。
...
$$\bold{x}^{(1)}(t)=\begin{pmatrix}
1\\1\\11
\end{pmatrix}e^{2t},\bold{x}^{(t)}(t)=\begin{pmatrix}
1\\0\\-1
\end{pmatrix}e^{-t},\bold{x}^{(3)}=\begin{pmatrix}
0\\1\\-1
...
这个例子中特征值 $r=-1$ 的代数重数是 2，仍旧有可能找到两个线性无关的特征向量 $\bold{\xi}^{(2)},\bold{\xi}^{(3)}$，最终构造了通解 $(34)$。

解 $(34)$ 的行为依赖于初始条件。对于 $t$ 很大的时候，由于 $(34)$ 的第一项指数是整数，那么占主导地位。因此 $c_1\neq 0$ 时，随着 $t\to\infty$，$\bold{x}$ 会无穷大。对于使得 $c_1=0$ 的初始条件，只有指数为负数的项，那么随着 $t\to\infty$，$\bold{x}\to\bold{0}$。使 $c_1=0$ 的初始点，正是位于由 $\bold{\xi}^{(2)},\bold{\xi}^{(3)}$ 确定的平面内的点。从这个平面开始的解，随着 $t\to\infty$ 都趋于原点而其他解趋于无穷。

如果一些特征值是共轭复数，它们都不相等，那么仍旧有 $n$ 个线性无关的解。复数特征值对应的解也是复数的。和 3.3 小节类似，还是可以转化成实数解，下一节会讨论这个问题。

如果特征值重复了，问题会复杂一些。此时，线性无关的特征向量少于特征值的代数重数，那么形如 $\bold{\xi}e^{rt}$ 的解的个数少于 $n$。为了构造出基础解系，有必要去找到其他形式的解。与 $n$ 阶常系数线性方程类似，特征方程的重复根对应的解的形式是 $e^{rt},te^{rt},t^2e^{rt},\cdots$。7.8 小节会讨论这种情况。

如果 $\bold{A}$ 是复数矩阵，那么复数特征值未必共轭出现，并且即使特征值是实数，对应的特征向量可能是复数向量。如果有 $n$ 个线性无关的特征向量，那么方程 $(1)$ 的解的形式仍旧如 $(27)$，不过通常解是复数的。
