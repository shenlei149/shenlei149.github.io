...
对于幂级数，存在一个正数 $\rho$ 称为收敛半径，比如 $\sum_{n=0}^\infty a_n(x-x_0)^n$ 在 $|x-x_0|<\rho$ 上绝对收敛，在 $|x-x_0|>\rho$ 上发散。区间 $|x-x_0|<\rho$ 称为收敛区间。如下图所示。TODO。在 $|x-x_0|=\rho$ 处可能收敛也可能发散。需要重要的幂级数对所有 $x$ 都收敛。这种情况 $\rho$ 无穷大，收敛区间是整个实轴。还有一种情况是仅在 $x_0$ 处收敛。这种情况下 $\rho=0$ 且没有收敛区间。统一考虑这些情况，每个幂级数有一个非负收敛半径 $\rho$，且如果 $\rho>0$ 那么存在有限或者无穷大的收敛区间。
