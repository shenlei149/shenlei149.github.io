这一节给出极限的精确定义。严格表述中需要避免“任意接近”这类描述性语言。

例1 考虑 $y=2x-1$ 在 $x=4$ 附近的情况。直觉上有 $\lim_{x\to 4}f(x)=7$。当 $x$ 位于4附近时，$x$ 要多接近4，才能使 $y=2x-1$ 与7的差小于两个单位？
解：问题本质就是使得 $|y-7|<2$？
$$|2x-1-7|=|2x-8|<2$$
那么
$$\begin{aligned}
...
但是这不足以说明极限，因为随着 $x$ 移向 $c$，无法保证 $f(x)$ 始终位于 $L$ 附近的区间 $L-(1/10),L+(1/10)$ 内并趋于 $L$；它可能跳到其他值。我们可以把误差区间缩小，例如取 $1/100,1/1000$，甚至 $1/100000$。每一次，我们都能在 $c$ 附近找到一个新的区间，使 $f(x)$ 满足这个误差要求。
...
这个过程可以概括为：对于每一个误差容限 $\varepsilon$，总能找到与之对应的 $\delta$，使得当 $x$ 充分接近 $c$ 时，$f(x)$ 的值位于区间 $L\pm\varepsilon$ 内。如下图所示：

![](030.030.png)

**定义**
> $f(x)$ 在 $c$ 附近的开区间有定义，除了 $c$ 点外。如果对于每一个 $\varepsilon>0$ 都有一个相应的 $\delta>0$ 使得
...
例2 证明
$$\lim_{x\to 1}(5x-3)=2$$
证明：令 $c=1,f(x)=5x-3,L=2$。对于任意给定的 $\varepsilon>0$，我们需要找到一个合适的 $\delta>0$，使得 $x$ 在区间
$$0<|x-1|<\delta$$
那么 $f(x)$ 在 $L$ 附近区间满足
$$|f(x)-2|<\varepsilon$$
我们通过解包含 $\varepsilon$ 的不等式来计算 $\delta$：
$$\begin{aligned}
...
$\delta=\varepsilon/5$ 不是唯一使得 $0<|x-1|<\delta$ 蕴涵 $|5x-5|<\varepsilon$。任意更小的 $\delta$ 都是可以的。定义并不要求找到最佳的 $\delta$，只要存在一个即可。
...
那么取 $\delta=\varepsilon$ 或者更小的正数即可。如下图所示

![](030.060.png)

（b）令 $\varepsilon>0$，我们需要找到 $\delta>0$ 有
$$|k-k|<\varepsilon \text{ whenever } 0<|x-c|<\delta$$
由于 $k-k=0$，任意正数 $\delta$ 都满足。如下图所示：

![](030.070.png)

### 给定 $\varepsilon$ 计算 $\delta$
...
解：解不等式 $|\sqrt{x-1}-2|<1$，寻找包含 $x=5$ 的区间，使其中除 $x=5$ 外的 $x$ 都满足不等式。
$$|\sqrt{x-1}-2|<1$$
$$-1<\sqrt{x-1}-2<1$$
$$1<\sqrt{x-1}<3$$
$$1<x-1<9$$
...
给定 $f,L,c$ 和 $\varepsilon>0$，如何计算 $\delta$
计算 $\delta>0$ 使得
$$|f(x)-L|<\varepsilon \text{ whenever } 0<|x-c|<\delta$$
的过程分为以下两步：
1. 解不等式 $|f(x)-L|<\varepsilon$，找到一个包含 $c$ 的开区间 $(a,b)$，使其中所有 $x\neq c$ 都满足不等式。$x=c$ 是否满足不等式并不重要。
...
寻找距离较近的端点，即 $\delta=\min\{2-\sqrt{4-\varepsilon},\sqrt{4+\varepsilon}-2\}$。
如果 $\varepsilon\geq4$，那么区间是 $(0,\sqrt{4+\varepsilon})$，可以取 $\delta=\min\{2,\sqrt{4+\varepsilon}-2\}$。
...
类似地，由于 $\lim_{x\to c}g(x)=M$，那么存在 $\delta_2>0$，使得
...
例7 证明乘法规则。
证明：对于任意 $\varepsilon>0$，我们需要找到 $\delta>0$，使定义域交集中任意满足 $0<|x-c|<\delta$ 的 $x$ 都有
