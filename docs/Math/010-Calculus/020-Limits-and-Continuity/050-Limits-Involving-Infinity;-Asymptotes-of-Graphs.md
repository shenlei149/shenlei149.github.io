### $x\to\pm\infty$ 时的极限
无穷符号 $\infty$ 不表示任何实数。例如，函数 $f(x)=1/x$（$x\neq0$）的图像如下图所示。当 $x$ 增大时，$1/x$ 减小；当 $x$ 为负数且绝对值增大时，情形类似。也就是说，当 $x\to\infty$ 或 $x\to-\infty$ 时，$f(x)=1/x$ 的极限是0。换言之，0是 $f(x)=1/x$ 在正无穷和负无穷处的极限。

![](050.010.png)

**定义**
> 如果对于所有 $\varepsilon>0$，都有对应的 $M$，使得对 $f$ 定义域中所有满足 $x>M$ 的 $x$，都有
> $$|f(x)-L|<\varepsilon, \text{ whenever } x>M$$
> 那么我们说 $x$ 趋于无穷时，$f(x)$ 的极限是 $L$，写作
> $$\lim_{x\to\infty}f(x)=L$$
> 如果对于所有 $\varepsilon>0$，都有对应的 $N$，使得对 $f$ 定义域中所有满足 $x<N$ 的 $x$，都有
> $$|f(x)-L|<\varepsilon, \text{ whenever } x<N$$
> 那么我们说 $x$ 趋于负无穷时，$f(x)$ 的极限是 $L$，写作
> $$\lim_{x\to -\infty}f(x)=L$$
直观地，如果 $\lim_{x\to\infty}f(x)=L$，那么当 $x$ 在正方向上离原点越来越远时，$f(x)$ 无限接近 $L$。类似地，如果 $\lim_{x\to-\infty}f(x)=L$，那么当 $x$ 沿负半轴离原点越来越远时，$f(x)$ 无限接近 $L$。
...
那么 $N=-1/\varepsilon$，或者任意更小的负数。
**定理8**
> 定理1的所有法则对 $x\to\infty,x\to-\infty$ 时同样适用。
...
> $$\lim_{x\to -\infty}f(x)=b$$
> 那么 $y=b$ 是 $f(x)$ 的水平渐近线。
函数图像可能有零、一条或两条水平渐近线，这取决于函数在 $x\to\infty$ 和 $x\to-\infty$ 时的极限。
图 2.36 的函数
...
### 无穷极限
回顾函数 $f(x)=1/x$。当 $x\to0^+$ 时，$f$ 的值无限增长。给定任意实数 $B$，总能找到 $f$ 的函数值大于 $B$。如下图所示：
![](050.100.png)
因此 $f$ 在 $x\to 0^+$ 时没有极限。然而，为了描述函数 $f$ 在 $x\to 0^+$ 时的行为，我们称 $f(x)$ 趋于无穷 $\infty$。记作
$$\lim_{x\to 0^+}f(x)=\lim_{x\to 0^+}\frac{1}{x}=\infty$$
这么写并不是说极限存在，也不是说 $\infty$ 是某个实数。这个式子只是在描述函数的行为：当 $x\to0^+$ 时，$\frac{1}{x}$ 的通常极限不存在，但函数值可以任意大。
$x\to 0^-$ 时类似，不再赘述。
例11 求
$$\lim_{x\to 1^+}\frac{1}{x-1},\lim_{x\to 1^-}\frac{1}{x-1}$$
解：从几何角度分析，函数 $\frac{1}{x-1}$ 是函数 $\frac{1}{x}$ 向右平移一个单位，因此它在1附近的行为与 $\frac{1}{x}$ 在0附近的行为一致。那么
...
> 或
> $$\lim_{x\to a^-}f(x)=\pm\infty$$
> 其中，只要函数值沿一个方向无界增长，那么 $x=a$ 就是函数 $y=f(x)$ 图像的垂直渐近线。
...
例18 自然对数图像的垂直渐近线是 $y$ 轴。从下图可以看出，关于 $y=x$ 对称的图像是指数函数 $y=e^x$，其水平渐近线是 $x$ 轴。
...
我们说 $x\to\pm\infty$ 时 $(x/2)+1$ 占主导地位，而 $x\to2$ 时 $1/(2x-4)$ 占主导地位。主项（`dominant terms`）能够帮助我们预测函数的行为。

例20 令 $f(x)=3x^4-2x^3+3x^2-5x+6,g(x)=3x^4$。证明尽管 $f,g$ 在 $|x|$ 不大时差异很大，但是当 $|x|$ 很大时，两者基本一致，即 $x\to\pm\infty$ 时二者的比值趋于1。
