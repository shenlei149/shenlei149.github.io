...
函数 $f(x)=x/|x|$（如下图所示）的右极限是1，左极限是-1。由于两个单侧极限不相等，所以当 $x\to0$ 时双侧极限不存在。
...
**定理6**
> 假设 $f$ 在包含 $c$ 的开区间有定义，$c$ 本身可以除外，那么 $f(x)$ 在 $c$ 处有极限等价于存在左极限和右极限且二者相等。
> $$\lim_{x\to c} f(x)=L \Leftrightarrow \lim_{x\to c^-}f(x)=L \text{ and } \lim_{x\to c^+}f(x)=L$$
> 在定义域的端点处，只要相应的单侧极限存在，我们就称该点处的极限存在。
...
$x=1$ 处，$\lim_{x\to 1^-}f(x)=0, \lim_{x\to 1^+}f(x)=1$，$\lim_{x\to 1}f(x)$ 不存在；
...
解：随着 $x$ 趋于0，$1/x$ 趋于无穷，$\sin(1/x)$ 在-1到1之间反复振荡，因此函数值不会稳定地接近某个值 $L$。上述分析对接近0的正 $x$ 和负 $x$ 都成立，即这个函数既没有左极限也没有右极限。
...
**定理7**
> 当 $\theta\to 0$ 时，$(\sin\theta)/\theta$ 的极限是
> $$\lim_{\theta\to 0}\frac{\sin\theta}{\theta}=1\tag{1}$$

证明：应用夹逼定理证明左右极限都是1，那么双侧极限就是1。
先在 $0<\theta<\frac{\pi}{2}$ 的条件下证明右极限是1。如下图所示
...
使用 $\theta$ 表示它们的面积
$$S_{\triangle OAP}=\frac{1}{2}(1)(1)\sin\theta=\frac{\sin\theta}{2}$$
...
&=-\lim_{y\to 0}\frac{\sin(y/2)}{y/2}\sin(y/2)\\
&=-(1)(0)
\end{aligned}$$
（b）分子分母同乘 $2/5$，那么
$$\begin{aligned}
...
### 证明夹逼定理
首先证明右极限。由于 $\lim_{x\to c^+}g(x)=\lim_{x\to c^+}h(x)=L$，那么对于任意 $\varepsilon>0$ 都存在 $\delta>0$，使得在区间 $(c,c+\delta)$ 上有
...
接着证明左极限。与上面类似，唯一的不同是这里 $x\in(c-\delta,c)$。
