根据函数值绘制图像时，往往使用连续曲线把各个点连起来，以此为未测量的点赋予对应的值，如下图所示。连续函数的输出规律而连贯，不会突然跳跃。直观地，在定义域的每个区间上，图像可以不抬笔画出的函数 $y=f(x)$ 都是连续函数。这些函数在研究微积分及其应用时很有用。
...
在 $x=1$ 处，函数没有极限。左极限 $\lim_{x\to1^-}f(x)=0$，右极限 $\lim_{x\to1^+}f(x)=1$，两者不相同，因此图像上有一个跳跃。函数在 $x=1$ 处不连续。不过，由于 $f(1)=1$ 与右极限相等，函数从 $x=1$ 起向右连续。
$x=2$ 处，函数存在极限 $\lim_{x\to2}f(x)=1$，但是不等于 $f(2)=2$，即极限值不等于函数值。因此在 $x=2$ 处不连续。
...
**连续性测试**
> 函数 $f(x)$ 在 $c$ 处连续，当且仅当满足下面三个条件：
>
> 1. $f(c)$ 存在；
> 2. $\lim_{x\to c}f(x)$ 存在；
> 3. $\lim_{x\to c}f(x)=f(c)$。
>
> 对于单边连续性，只需将条件 2 和条件 3 中的极限改为相应的单边极限。

例4 函数 $y=\lfloor x\rfloor$ 如下图所示。

![](060.050.png)

在每个整数处都不连续，因为当 $x\to n$ 时左极限不等于右极限。
...
（b）恒等函数 $f(x)=x$ 和常值函数 $f(x)=k$ 都是连续函数。
**定理9 连续函数的属性**
...
&=\sin c
\end{aligned}$$
因此，$\sin x$ 在点 $c$ 处连续。$c$ 可以是任意值，所以 $\sin x$ 处处连续。
...
### 反函数和连续性
如果一个连续函数在某个区间上存在反函数，那么反函数在自己的定义域上也是连续的。观察 $f^{-1}$ 的图像：它和 $f$ 的图像关于 $y=x$ 对称；如果原函数没有间断，那么反函数也不会有间断。严格证明需要更高等的数学知识。由此可知六个反三角函数在其定义域上都是连续的。
...
定理10比较直观。因为 $x$ 趋近 $c$，所以 $f(x)$ 趋近 $f(c)$；由于 $g$ 在 $f(c)$ 处连续，所以 $g(f(x))$ 趋于 $g(f(c))$。
...
证明：
（a）根据定理9，根函数在其定义域上连续。多项式 $f(x)=x^2-2x-5$ 是连续函数，因此 $g(x)=\sqrt{f(x)}$ 在其定义域上连续。
（b）分子是恒等函数的平方的立方根，是连续函数；分母是处处是正数的多项式函数，也是连续函数。商也是连续的。
（c）商 $(x-2)/(x^2-2)$ 在 $x\neq\pm\sqrt{2}$ 处连续，例7告诉我们绝对值函数是连续的。
...
$$|g(y)-g(b)|<\varepsilon, \text{ whenever } |y-b|<\delta_1$$
...
证明：令 $f(x)=\sqrt{2x+5}+x^2-4,g(x)=\sqrt{2x+5}$。首先证明连续性。$g(x)$ 是根函数和多项式函数的复合函数，所以在定义域 $[-5/2,\infty)$ 上连续；$f(x)$ 是 $g(x)$ 和一个多项式函数的和，所以也连续。$f(0)=\sqrt{5}-4<0,f(2)=3+4-4=3>0$，而 $[0,2]\subset[-5/2,\infty)$，所以在0和2之间存在某个 $c$，使得 $f(c)=0$；这个 $c$ 就是原方程的解。
...
在 $x=2$ 处有连续延拓，求这个延拓。
解：尽管 $x=2$ 处没有定义，但是当 $x\neq\pm2$ 时，
$$f(x)=\frac{x^2+x-6}{x^2-4}=\frac{(x-2)(x+3)}{(x-2)(x+2)}=\frac{x+3}{x+2}$$
新函数
$$F(x)=\frac{x+3}{x+2}$$
当 $x\neq\pm2$ 时，$F(x)=f(x)$，并且 $F$ 在 $x=2$ 处连续，值为 $5/4$。因此 $F$ 是 $f$ 在 $x=2$ 处的连续延拓，并且
