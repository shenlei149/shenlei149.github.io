...
> 7. 根法则：$\lim_{x\to c}\sqrt[n]{f(x)}=\sqrt[n]{L}=L^{1/n}$，其中 $n$ 是正整数；若 $n$ 是偶数，还需假设 $f(x)$ 在 $c$ 附近非负。
...
（c）
$$\begin{aligned}
\lim_{x\to -1}\sqrt{4x^2-3}&=\sqrt{\lim_{x\to -1}(4x^2-3)}\\
&=\sqrt{4(-1)^2-3}\\
&=1
\end{aligned}$$

### 多项式函数和有理函数的极限
定理1的极限法则大大简化了求多项式函数和有理函数极限的过程。对于多项式函数，将 $x=c$ 代入多项式即可。对于有理函数，如果 $c$ 不使分母为零，直接代入即可。
...
### 使用计算机估算极限
我们可以用计算机通过数值法估计极限。不过，计算机可能给出错误的值和误导性的信息，通常是由舍入误差造成的。

例8 估计
$$\lim_{x\to 0}\frac{\sqrt{x^2+100}-10}{x^2}$$
解：如下表，如果我们取 $\pm 1,\pm 0.5,\pm 0.1,\pm 0.01$ 来表示 $x$ 趋于0，那么函数值趋于0.05。
