...
$$y=4.9t^2$$
这里4.9是一个近似常数。
更一般地，设移动物体在时刻 $t$ 距起点的距离为 $f(t)$。在时间段 $[t_1,t_2]$ 内，物体的平均速度（`average speed`）定义为移动距离 $f(t_2)-f(t_1)$ 除以 $t_2-t_1$。单位可以是千米每小时或者米每秒。
...
我们不能用这个公式来计算瞬时速度，因为 $h$ 不能为零。我们可以计算从 $t_0=1,2$ 开始的很小时间段内的平均速度。只需减小 $h$ 即可。
...
### 平均变化率和割线
给定函数 $y=f(x)$，$x$ 的变化区间是 $[x_1,x_2]$，对应的 $y$ 的变化量是 $\Delta y=f(x_2)-f(x_1)$，那么平均变化率是 $\Delta y$ 除以 $\Delta x=x_2-x_1=h$。后面用 $h$ 简化表示 $x$ 的变化量 $\Delta x$。
...
解：割线的一个端点是 $P(2,4)$，另一个端点是 $Q(2+h,(2+h)^2)$。所以斜率为
$$\frac{\Delta y}{\Delta x}=\frac{(2+h)^2-2^2}{h}=\frac{h^2+4h}{h}=h+4$$
$h>0$，那么 $Q$ 在 $P$ 的右侧（如下图所示）。$h<0$ 时，$Q$ 在 $P$ 的左侧。不管哪种情况，随着 $Q$ 接近 $P$，$h$ 趋于0，那么斜率趋于4，所以曲线在 $P$ 处的斜率是4。
...
例4 下图显示了果蝇在 50 天实验中每天的数量 $p$。每次计数的间隔相同，各点用光滑曲线连接。求第 23 天到第 50 天的平均变化率。

![](010.050.png)

解：如图，23天时有150只果蝇，50天时有340只，所以平均变化率是
$$\frac{\Delta p}{\Delta t}=\frac{340-150}{50-23}=\frac{190}{27}\approx 7.0\text{ flies/day}$$
平均变化率是割线 $PQ$ 的斜率。
...
随着 $t$ 轴上 $Q$ 从45移动到30，割线的斜率从8.6上升到16.4，如果继续减小到23，那么斜率还会上升一点。从几何意义上看，割线绕 $P$ 点逆时针转动，最后与红线重合。由于红线除点 $P$ 外还经过点 $(14,0)$，所以其斜率近似为
