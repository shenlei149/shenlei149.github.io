...
定理 1 的一个推论是连续函数的组合函数在定义域交集上是连续的。在存在定义的点上，连续函数的和、差、倍、积等，包括多项式和有理函数，也都是连续的。
...
**组合函数的连续性**
> 如果函数 $f(x,y)$ 在 $(x_0,y_0)$ 是连续的，$g$ 是在点 $f(x_0,y_0)$ 处连续的一元函数，那么组合函数 $h=g\circ f=g(f(x,y))$ 在 $(x_0,y_0)$ 处是连续的。
